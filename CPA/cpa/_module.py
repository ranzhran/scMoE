...
def infonce_loss_cosine(expert_outputs, temperature=0.2):
    ...
    loss = F.binary_cross_entropy_with_logits(logits / temperature, labels)  # 使用二分类交叉熵计算损失
...
class MulMoE(nn.Module):
    ...
    def forward(self, feature_input, inputs):
        ...
        for i in range(self.n_heads):
            gating_outputs = self.gating_kernels[i](feature_input)
            if self.use_gating_bias:
            ...
            weighted_gating_outputs = gating_outputs * self.global_weights  

            top_k_values, top_k_indices = torch.topk(weighted_gating_outputs, k=6, dim=-1)  

            normalized_top_k_values = torch.softmax(top_k_values, dim=-1) 
            selected_experts = torch.gather(expert_outputs, -1, top_k_indices.unsqueeze(1).expand(-1, self.units, -1))  

            weighted_experts = selected_experts * normalized_top_k_values.unsqueeze(1)

            head_output = weighted_experts.sum(dim=-1)  
...
class CPAModule(BaseModuleClass):
    ...
    def __init__(self,
        ...
        if variational:
            ...
            self.encoder = VanillaEncoder(
                n_input=512,#n_genes zheli 2048 best 11319+2048
                n_output=n_latent,
                n_cat_list=[],
                n_hidden=n_hidden_encoder,
                n_layers=n_layers_encoder,
        ...
        self.expert_input_dims = [512, 512, 256, 1536, 3072, 512, 1120, 3799]
        self.moe_layer = MulMoE(units=1024, n_experts=8, expert_input_dims=self.expert_input_dims, feature_dim=self.n_genes, n_heads=8)  #self.n_genes
        self.moe_nn = nn.Sequential(
                nn.Linear(1024*8, 1024*4),
                nn.BatchNorm1d(1024*4),
                nn.ReLU(),
                nn.Dropout(p=0.2),
                nn.Linear(1024*4, 1024*2) #1024*2
                )
    ...
    def _get_inference_input(self, tensors):
        ...
        x_emb = tensors[CPA_REGISTRY_KEYS.EMB]  # batch_size, n_genes
        perts = {
    ...
    def inference(
        ...
        # moe+cat
        # z_cat = self.emb_encoder(x_emb)
        # z_pre = self.moe_layer(torch.tensor(x_), torch.tensor(x_emb))
        # z_pre = self.moe_nn(torch.cat((z_cat, z_pre), dim=1))



        # concat
        # z_pretrained = self.emb_encoder(x_emb)

        # 1012最优
        # z_pre, expert_outputs = self.moe_layer(torch.tensor(x_), torch.tensor(x_emb))
        # z_pre = self.moe_nn(z_pre)

        # z_pre = torch.cat((torch.tensor(x_emb), z_pre), dim=-1)
        ...
        # single cell
        if self.variational:
            qz, z_basal = self.encoder(x_emb)
        else:
            qz, z_basal = None, self.encoder(x_emb)

        # moe
        # if self.variational:
        #     qz, z_basal = self.encoder(z_pre)
        # else:
        #     qz, z_basal = None, self.encoder(z_pre)
        ...
        # print("perts['true']",perts['true'][0])
        # print("perts_doses['true']",perts_doses['true'][0])
        # print('======================')
        if covars_to_add is None:
            ...
            # print("call covars_to_add=========================")
            perts_test = torch.tensor(covars_to_add[0]).unsqueeze(0).repeat(batch_size, 1).cuda()
            perts_doses_test = torch.tensor(covars_to_add[1]).unsqueeze(0).repeat(batch_size, 1).cuda()
            z_pert_true = self.pert_network(perts_test, perts_doses_test)
            z_pert = z_pert_true
        ...
        # z = z_basal + z_pretrained + z_pert + z_covs
        # z_corrected = z_basal + z_pretrained + z_pert + z_covs_wo_batch
        # z_no_pert = z_basal + z_pretrained + z_covs
        # z_no_pert_corrected = z_basal + z_pretrained + z_covs_wo_batch
    ...
    def loss(self, tensors, inference_outputs, generative_outputs):
        ...
        eo = inference_outputs["eo"]
        info_loss = infonce_loss_cosine(eo)
        ...
        return recon_loss, kl_loss, info_loss
