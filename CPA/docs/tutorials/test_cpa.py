...
# counts_matrix = adata.layers['counts'].toarray()  # 如果是稀疏矩阵，可以使用 .toarray() 转换为密集矩阵

# gene_names = adata.var_names

# print(gene_names)
# print(counts_matrix.size)
# print('===============')
...
model.train(max_epochs=100, #2000
            ...
            save_path='/home/zhangran/scdrug/cpa/lightning_logs/combo/cpa/',
           )
...
# df.to_csv('log0820/results/cpa_base.csv', index=True) 

df = pd.DataFrame(results)
mean_results = df.groupby('n_top_deg').mean()

print(mean_results)

mean_results.to_csv('log0821/mean_results/cpa_base.csv', index=True)

df.to_csv('log0821/results/cpa_base.csv', index=True) 